...
local MiniMapIcon = CreateFrame("Button", "LibDBIcon-1.0", Minimap)
...
MiniMapIcon.icon:SetTexture("Interface\\AddOns\\GDKPd\\media\\PlusButton")
...
MiniMapIcon.iconMini:SetTexture("Interface\\AddOns\\GDKPd\\media\\PlusButton")
...
MiniMapIcon:SetScript("OnEnter",function(self) 
	...
	GameTooltip:AddLine("Gold DKP\nЛКМ-настройки\nПКМ-Спрятать окна") 
	GameTooltip:AddLine(minimaptooltiplmp,1,1,1) 
	GameTooltip:AddLine(minimaptooltiprmp,1,1,1) 
	GameTooltip:Show() 
	self.anim:Play()
	self.iconMini:Show()
end)
...
local function IconMoveButton(self)
	if self.dragMode == "free" then
		...
		ExRT.IconMiniMapLeft = x
		ExRT.Addon.IconMiniMapTop = y
	else
...
status:Hide()
status:SetBackdrop({
	bgFile="Interface\\AddOns\\GDKPd\\media\\background",
...
function GDKPd:AuctionOffItem(item, minbid, increment)
	...
	if (not self.opt.allowMultipleAuctions) then
		...
		-- new code
		SendChatMessage((L["[GDKPd] Bidding starts on %s. Bid using format '[item] 1000', starting bid %d gold, minimum increment %d gold. TTL: %d/%d"]):format(item,minbid,increment,self.opt.auctionTimer, self.opt.auctionTimerRefresh), (self.opt.announceRaidWarning and (IsRaidOfficer() or IsRaidLeader())) and "RAID_WARNING" or "RAID")
...
function GDKPd:RevertHighestBid(link)
	if self.opt.allowMultipleAuctions then
		...
		SendChatMessage((L["[GDKPd] New highest bidder on %s: %s (%d gold)"]):format(link, aucdata.bidders[1].bidderName, aucdata.bidders[1].bidAmount), (self.opt.announceBidRaidWarning and (IsRaidOfficer() or IsRaidLeader())) and "RAID_WARNING" or "RAID")
		...
		SendChatMessage((L["[GDKPd] New highest bidder: %s (%d gold)"]):format(self.curAuction.bidders[1].bidderName, self.curAuction.bidders[1].bidAmount), (self.opt.announceBidRaidWarning and (IsRaidOfficer() or IsRaidLeader())) and "RAID_WARNING" or "RAID")
...
function GDKPd:CancelAuction(link)
	if self.opt.allowMultipleAuctions then
		...
		local aucdata = self.curAuctions[link]
		if not aucdata then return end
		SendChatMessage((L["[GDKPd] Auction cancelled for %s."]):format(link), (self.opt.announceRaidWarning and (IsRaidOfficer() or IsRaidLeader())) and "RAID_WARNING" or "RAID")
		self.curAuctions[link] = nil
	else
		SendChatMessage(L["[GDKPd] Auction cancelled."], (self.opt.announceRaidWarning and (IsRaidOfficer() or IsRaidLeader())) and "RAID_WARNING" or "RAID")
	...
	if self.opt.allowMultipleAuctions then
		...
		if aucdata then
			...
			if aucdata.bidders[1] then
				...
				SendChatMessage((L["[GDKPd] Auction finished for %s. Winner: %s. %s."]):format(link, aucdata.bidders[1].bidderName, paymentString),"RAID")
				...
				if self.opt.announcePotAfterAuction then
					SendChatMessage(L["[GDKPd] Current pot: "]..GDKPd_PotData.potAmount..L[" gold"],"RAID")
				end
				...
				SendChatMessage((L["[GDKPd] Auction finished for %s. No bids recieved."]):format(link),"RAID")
			end
			aucdata:Release()
		end
		self.curAuctions[link] = nil
		...
		if self.curAuction.bidders[1] then
			...
			paymentString = paymentString:format(remAmount)
			SendChatMessage((L["[GDKPd] Auction finished. Winner: %s. %s."]):format(self.curAuction.bidders[1].bidderName, paymentString),"RAID")
			GDKPd_PotData.potAmount = (GDKPd_PotData.potAmount or 0)+remAmount
			...
			SendChatMessage(L["[GDKPd] Auction finished. No bids recieved."],"RAID")
		end
...
function GDKPd:DistributePot()
	...
	SendChatMessage((L["[GDKPd] Distributing pot. Pot size: %d gold. Amount to distribute: %d gold. Players in raid: %d. Share per player: %d gold."]):format((GDKPd_PotData.potAmount or 0), distAmount, numraid, (distAmount or 0)/numraid),"RAID")
...
GDKPd:SetScript("OnEvent", function(self, event, ...)
	...
	if (event == "CHAT_MSG_ADDON") then
		...
		if sender then
			if event == "CHAT_MSG_ADDON" and arg[1] == "GDKPD START" and self:PlayerIsML(sender,false) then
			...
			if event == "CHAT_MSG_ADDON" and arg[1] == ("GDKPD VREQ") then
				SendAddonMessage("GDKPD VDATA", DEBUGFORCEVERSION or "1.4.2", "WHISPER", arg[4])
			end
			if event == "CHAT_MSG_ADDON" and arg[1] == ("GDKPD VDATA") then
				self.versions[sender] = arg[2]
				self.version:Update()
			end
			if event == "CHAT_MSG_ADDON" and arg[1] == "GDKPD MANADJ" and self:PlayerIsML(sender,false) then
...
-- if (event == "CHAT_MSG_ADDON" and arg1 == ("GDKPD START")
-- if (event == "CHAT_MSG_ADDON" and arg1 == ("GDKPD VREQ")
-- if (event == "CHAT_MSG_ADDON" and arg1 == ("GDKPD VDATA")
-- if (event == "CHAT_MSG_ADDON" and arg1 == ("GDKPD MANADJ")
--prefixes done
